...
async def async_setup_entry(
    ...
    entities: list[AtaDeviceClimate | AtwDeviceZoneClimate] = [
        AtaDeviceClimate(mel_device, mel_device.device, coordinator)
        for mel_device in mel_devices.get(DEVICE_TYPE_ATA, [])
    ]
    entities.extend(
        [
            AtwDeviceZoneClimate(mel_device, mel_device.device, zone, coordinator)
...
class MelCloudClimate(ClimateEntity):
    ...
    def __init__(
        self, device: MelCloudDevice, coordinator: MelCloudDataUpdateCoordinator
    ) -> None:
    ...
    def available(self) -> bool:
        ...
        return self.coordinator.last_update_success and self.api.available
